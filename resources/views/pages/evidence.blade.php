<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto mt-8 pb-8 grid 2xl:grid-cols-2 xl:grid-cols-2 lg:grid-cols-2 lg:gap-x-4 md:grid-cols-1 md:gap-y-8 gap-x-6 md:px-3 xmd:gap-y-6">
        ...
        <form action="{{ route('notes.store') }}" method="post">
            @csrf
            <div
                class="bg-white overflow-hidden shadow rounded-lg grid md:grid-cols-2 lg:gap-x-4 sm:gap-x-4 lg:grid-cols-2 grid-flow-row gap-y-3 py-4 px-4 drop-shadow-2xl">
                <h2 class="lg:col-span-2 md:col-span-2 text-2xl">Student Note</h2>
                <x-jet-label for="studentnote" class="text-lg col-start-1 md:col-span-1 xmd:col-span-2">Select Student</x-jet-label>
                    <select id="studentnote" name="student_id" required
                        class="border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm lg:col-span-1 lg:col-start-1 md:col-span-1 md:col-start-1 md:self-start">
                    ...
                    <x-jet-label for="filelink" class="text-lg lg:row-start-2 md:col-end-3 sm:row-start-2 xmd:col-start-2 xmd:row-start-1">
