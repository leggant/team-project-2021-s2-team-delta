<x-app-layout>
    ...
    <div class="max-w-7xl min-w-2xl mx-auto sm:px-8 lg:px-8 mt-8 grid grid-cols-2 gap-6 pb-8">
        ...
        <form action="{{ route('students.update',  $student->id) }}" method="POST"
            ...
            @csrf
            {{ method_field('PUT') }}
            <x-jet-label for="edit_first_name">First Name</x-jet-input>
                <x-jet-input type="text" id="edit_first_name" name="first_name" value="{{ $student->first_name }}"
                    required aria-label="Student First Name" class="col-span-2"></x-jet-input>
                <x-jet-label for="edit_last_name">Last Name</x-jet-label>
                <x-jet-input type="text" id="edit_last_name" name="last_name" value="{{ $student->last_name }}"
                    required placeholder="Student Last Name" aria-label="Student Last Name" class="col-span-2"></x-jet-input>
                <x-jet-label for="edit_id">User Name</x-jet-label>
                ...
                <x-jet-label for="edit_github">Edit GitHub</x-jet-label>
                <x-jet-input type="text" id="edit_github" name="github" value="{{ $student->github }}" placeholder="Github Username" aria-label="Github Username" class="col-span-2"></x-jet-input>
            ...
            @if($uploads->count() > 0)
                @foreach($uploads as $file)
                    <div class="grid grid-cols-6 mt-3 place-items-start items-center gap-2">
                        ...
                        <form method="post"
                            action="{{ route('evidence.destroy', $file->id) }}">
                            @csrf
                            @method('delete')
                            <x-delete-button />
                        </form>
        ...
        <div>
            <form action="{{ route('evidence.store', $student->id) }}" method="post"
                ...
                <div
                    ...
                    <x-jet-input type="text" name="title" id="title" value="{{ old('title') }}"
                        required aria-label="Upload Title" aria-placeholder="Enter A Upload Title"
            ...
            <form action="{{ route('notes.store') }}" method="post">
                ...
                <div
                    ...
                    <x-jet-input type="url" name="filelink" id="filelink" placeholder="Enter File URL"
                        ...
                        class="focus:placeholder-gray-400 lg:col-start-2 lg:col-end-3"
                        value="{{ old('filelink') }}">
                    </x-jet-input>
