...
<div class="grid max-w-7xl 2xl:grid-cols-2 xl:grid-cols-2 lg:grid-cols-2 md:grid-cols-2 md:mx-auto xmd:grid-cols-2 sm:grid-cols-1 xs:grid-cols-1 mb-6 mx-auto gap-x-6 gap-y-6 sm:w-full content-start">
    ...
    <details
        ...
        <summary dusk="dropdown_{{ $up->paper_name }}">
            ...
            <tr>
                <td class="border py-2 px-2 text-center hover:cursor-pointer">
                    <input type="checkbox" name="student_checkboxes[]" value="{{$student->id}}" id="checkbox_students"
                        dusk="student_checkboxes{{$student->id}}" required class="hover:cursor-pointer px-2 py-2" />
                </td>
        ...
        <div class="grid gap-x-6 gap-y-6 grid-rows-2">
            ...
            <form id="move" action="{{route('move')}}" method="POST" class="flex gap-x-4">
                ...
                <x-jet-button onclick="move({{$oldcount}})" dusk="transfer">Transfer</x-jet-button>
            </form>
        </div>
