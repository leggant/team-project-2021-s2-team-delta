...
class PagesController extends Controller
{
   ...
   public function getStudent($id) {
    ...
    if ($students->where('id', $id)->exists()) {
        ...
        $evidences = DB::table('evidence')->where('student_id', 'LIKE', '%' . $student->id . '%')->get();
        $notes = DB::table('notes')->where('student_id', 'LIKE', '%' . $student->id . '%')->get();
        # Will also have to pass respective evidence and notes/observations rows here once they have proper relationships
        return view('pages.viewStudent', ['student'=>$student, 'evidences'=>$evidences, 'notes'=>$notes]);
    } else {
    ...
    public function notes(){
        ...
        $student = DB::select('select * from student'); 
        return view('pages.notes',['notes'=>$notes], ['student'=>$student]);
    }
}
