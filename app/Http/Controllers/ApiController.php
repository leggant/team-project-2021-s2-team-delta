...
class ApiController extends Controller
{
    ...
    public function updateCohort(Request $request, $id) {
        ...
        if ($cohorts->where('id', $id)->exists()) {
            ...
            $cohort->subject = is_null($request->subject) ? $cohort->subject : $request->subject;
            $cohort->year = is_null($request->year) ? $cohort->year : $request->year;
            $cohort->semester = is_null($request->semester) ? $cohort->semester : $request->semester;
            $cohort->stream = is_null($request->stream) ? $cohort->stream : $request->stream;
            $cohort->students = is_null($request->students) ? $cohort->students : $request->students;
            $cohort->save();
